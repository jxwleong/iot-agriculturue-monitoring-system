...
#define DHTPIN_1           D2       // Input Data pin for dht sensor 1
#define DHTTYPE    DHT11            // DHT 11

#define DHTPIN_2           D4       // Input Data pin for dht sensor 2
#define DHTPIN_3           D5       // Input Data pin for dht sensor 3
#define DHTPIN_4           D6       // Input Data pin for dht sensor 4
 ...
 */
DhtData getDhtSensorReadings(int pin){
  DhtData dhtData;

  if (dht.read(DHT11, pin)){ // If no eror then status return 1
 ...
 */
SensorData getSensorData(int pin){
  SensorData sensorData;
  // Get all sensor data
  sensorData.dht11Data = getDhtSensorReadings(pin);
 ...
 */
char *createJsonStringForSensorReadings(float temperature){
 ...
 */
void sendSensorReadingsToServerNode(float temperature){

  char *message = createJsonStringForSensorReadings(temperature);
  Serial.println("\nPublishing message:");
...
void ISR_FUNC(){
  ...
  SensorData sensorData1, sensorData2, sensorData3, sensorData4;
  Serial.println("\n========================================");
  Serial.println("  ISR to collect and send sensor data.  ");
  Serial.println("========================================");
  Serial.println("Collecting sensor data now...");
  sensorData1 = getSensorData(DHTPIN_1);
  sensorData2 = getSensorData(DHTPIN_2);
  sensorData3 = getSensorData(DHTPIN_3);
  sensorData4 = getSensorData(DHTPIN_4);
  
  sendSensorReadingsToServerNode(sensorData1.dht11Data.temperature);
  sendSensorReadingsToServerNode(sensorData2.dht11Data.temperature);
  sendSensorReadingsToServerNode(sensorData3.dht11Data.temperature);
  sendSensorReadingsToServerNode(sensorData4.dht11Data.temperature);
  } 
}
